...
export const Player: React.FC<PlayerProps> = ({
  ...
  *
  * Props:
  * - playerData: Determines visual appearance and possibly abilities
  * - username: Unique identifier displayed above character
  * - position: Initial spawn coordinates
  * - color: Optional custom color for character
  * - isLocal: Boolean determining if this is the user-controlled player
 ...
 // --- Configuration ---
 const PLAYER_CONFIG = {
     SPEED: 5.0, // Match server logic
     SPRINT_MULTIPLIER: 1.8, // Match server logic
     ANIMATION_CROSSFADE_DURATION: 0.2,
 };

 // --- Client-side Prediction Constants ---
 const PREDICTION_CONFIG = {
     SERVER_TICK_RATE: 60, // Assuming server runs at 60Hz
     POSITION_RECONCILE_THRESHOLD: 0.4,
     ROTATION_RECONCILE_THRESHOLD: 0.1, // Radians
     RECONCILE_LERP_FACTOR: 0.15,
 };

 // --- Camera Constants ---
 const CAMERA_CONFIG = {
     FOLLOW_ZOOM_SPEED: 0.8,
     FOLLOW_MIN_ZOOM: 2.0,
     FOLLOW_MAX_ZOOM: 12.0,
     FOLLOW_CAMERA_HEIGHT: 2.5,
     FOLLOW_LOOK_HEIGHT: 1.8,
     FOLLOW_DAMPING: 12,
     ORBITAL_ZOOM_SPEED: 0.5,
     ORBITAL_DISTANCE_MIN: 3,
     ORBITAL_DISTANCE_MAX: 20,
     ORBITAL_HEIGHT_OFFSET: 3,
     ORBITAL_ELEVATION_DEFAULT: Math.PI / 6, // Approximately 30 degrees
     ORBITAL_AUTO_ROTATE_SPEED: 0.5,
     ORBITAL_DAMPING: 8,
 };
 ...
 // Custom Hook for Animation Loading and Management
 const usePlayerAnimations = (model: THREE.Group | null, characterClass: string) => {
     const [mixer, setMixer] = useState<THREE.AnimationMixer | null>(null);
     const [animations, setAnimations] = useState<Record<string, THREE.AnimationAction>>({});
     const [currentAnimation, setCurrentAnimation] = useState<string>(ANIMATIONS.IDLE);
     const animationsLoadedRef = useRef(false);
     ...
     useEffect(() => {
         if (model && !animationsLoadedRef.current) {
             animationsLoadedRef.current = true;
             const newMixer = new THREE.AnimationMixer(model);
             setMixer(newMixer);
             loadAnimations(newMixer, characterClass, setAnimations);
         }
     }, [model, characterClass]);

     const loadAnimations = useCallback((mixerInstance: THREE.AnimationMixer, charClass: string, setAnims: React.Dispatch<React.SetStateAction<Record<string, THREE.AnimationAction>>>) => {
         if (!mixerInstance) return;

         const animationPaths: Record<string, string> = {};
         const basePath = charClass === 'Paladin' ? '/models/paladin/' : '/models/wizard/';
         const animKeys = {
             idle: charClass === 'Wizard' ? 'wizard-standing-idle.fbx' : 'paladin-idle.fbx',
             'walk-forward': charClass === 'Wizard' ? 'wizard-standing-walk-forward.fbx' : 'paladin-walk-forward.fbx',
             'walk-back': charClass === 'Wizard' ? 'wizard-standing-walk-back.fbx' : 'paladin-walk-back.fbx',
             'walk-left': charClass === 'Wizard' ? 'wizard-standing-walk-left.fbx' : 'paladin-walk-left.fbx',
             'walk-right': charClass === 'Wizard' ? 'wizard-standing-walk-right.fbx' : 'paladin-walk-right.fbx',
             'run-forward': charClass === 'Wizard' ? 'wizard-standing-run-forward.fbx' : 'paladin-run-forward.fbx',
             'run-back': charClass === 'Wizard' ? 'wizard-standing-run-back.fbx' : 'paladin-run-back.fbx',
             'run-left': charClass === 'Wizard' ? 'wizard-standing-run-left.fbx' : 'paladin-run-left.fbx',
             'run-right': charClass === 'Wizard' ? 'wizard-standing-run-right.fbx' : 'paladin-run-right.fbx',
             jump: charClass === 'Wizard' ? 'wizard-standing-jump.fbx' : 'paladin-jump.fbx',
             attack1: charClass === 'Wizard' ? 'wizard-standing-1h-magic-attack-01.fbx' : 'paladin-attack.fbx',
             cast: charClass === 'Wizard' ? 'wizard-standing-2h-magic-area-attack-02.fbx' : 'paladin-cast.fbx',
             damage: charClass === 'Wizard' ? 'wizard-standing-react-small-from-front.fbx' : 'paladin-damage.fbx',
             death: charClass === 'Wizard' ? 'wizard-standing-react-death-backward.fbx' : 'paladin-death.fbx',
         };

         Object.entries(animKeys).forEach(([key, filename]) => {
             animationPaths[key] = `<span class="math-inline">\{basePath\}</span>{filename}`;
         });
         ...
         const checkCompletedLoading = () => {
             loadedCount++;
             if (loadedCount === totalCount) {
                 setAnims(newAnimations);
                 if (newAnimations['idle']) {
                     setTimeout(() => {
                         if (animationsLoadedRef.current) {
                             playAnimationInternal('idle', PLAYER_CONFIG.ANIMATION_CROSSFADE_DURATION, newAnimations, mixerInstance, setCurrentAnimation);
                         }
                     }, 100);
                 } else {
         ...
         const loadAnimationFile = (name: string, path: string, mixerInst: THREE.AnimationMixer) => {
             if (!mixerInst) {
                 checkCompletedLoading();
                 return;
             }
             ...
             if (animationCache[path]) {
                 const clip = animationCache[path].clone();
                 clip.name = name;
                 makeAnimationInPlace(clip);
                 newAnimations[name] = mixerInst.clipAction(clip);
                 checkCompletedLoading();
                 return;
             }
             ...
             loader.load(
                 ...
                 (animFbx) => {
                     ...
                     clip.name = name;
                     makeAnimationInPlace(clip);
                     const retargetedClip = retargetClip(clip, path, mainModelPath, model);
                     const action = mixerInst.clipAction(retargetedClip);
                     newAnimations[name] = action;
                     animationCache[path] = clip.clone();
         ...
         Object.entries(animationPaths).forEach(([name, path]) => loadAnimationFile(name, path, mixerInstance));
     }, [mainModelPath, model]);

     const playAnimationInternal = useCallback((name: string, crossfadeDuration: number, anims: Record<string, THREE.AnimationAction>, mixerInstance: THREE.AnimationMixer | null, setCurrentAnim: React.Dispatch<React.SetStateAction<string>>) => {
         if (!mixerInstance || !anims[name]) return;

         const targetAction = anims[name];
         const currentAction = anims[currentAnimation];
     ...
     const playAnimation = useCallback((name: string, crossfadeDuration = PLAYER_CONFIG.ANIMATION_CROSSFADE_DURATION) => {
         playAnimationInternal(name, crossfadeDuration, animations, mixer, setCurrentAnimation);
     }, [animations, mixer, setCurrentAnimation, playAnimationInternal]);
     ...
     return { mixer, animations, currentAnimation, playAnimation };
 };

 const retargetClip = (clip: THREE.AnimationClip, sourceModelPath: string, targetModelPath: string, targetModel: THREE.Group | null) => {
     if (!targetModel) return clip;

     const sourceFileName = sourceModelPath.split('/').pop()?.split('.')[0] || '';
     const targetFileName = targetModelPath.split('/').pop()?.split('.')[0] || '';
     ...
     clip.tracks.forEach(track => {
         ...
         // Example Bone Name Mapping
         // if (boneName.startsWith('bip01_')) {
         //     targetBoneName = boneName.replace('bip01_', 'mixamorig');
         // }

         const newTrackName = `<span class="math-inline">\{targetBoneName\}\.</span>{property}`;
         if (newTrackName !== track.name) {
             let newTrack: THREE.KeyframeTrack;
             if (track instanceof THREE.QuaternionKeyframeTrack) {
                 newTrack = new THREE.QuaternionKeyframeTrack(newTrackName, Array.from(track.times), Array.from(track.values));
             } else if (track instanceof THREE.VectorKeyframeTrack) {
                 newTrack = new THREE.VectorKeyframeTrack(newTrackName, Array.from(track.times), Array.from(track.values));
             } else {
                 newTrack = new THREE.KeyframeTrack(newTrackName, Array.from(track.times), Array.from(track.values));
             }
             newTracks.push(newTrack);
         } else {
             newTracks.push(track);
 ...
 const makeAnimationInPlace = (clip: THREE.AnimationClip) => {
     ...
     if (!rootTrack && positionTracks.length > 0) {
         rootTrack = positionTracks[0];
     }
 ...
 export const Player: React.FC<PlayerProps> = ({
     ...
     const { mixer, animations, currentAnimation, playAnimation } = usePlayerAnimations(model, characterClass);

     // --- Client Prediction State ---
     const localPositionRef = useRef<THREE.Vector3>(new THREE.Vector3(playerData.position.x, playerData.position.y, playerData.position.z));
     const localRotationRef = useRef<THREE.Euler>(new THREE.Euler(0, playerData.rotation.y, 0, 'YXZ'));
     const debugArrowRef = useRef<THREE.ArrowHelper | null>(null);

     // Camera control variables
     const isPointerLocked = useRef(false);
     const zoomLevel = useRef(CAMERA_CONFIG.FOLLOW_MIN_ZOOM);
     const targetZoom = useRef(CAMERA_CONFIG.FOLLOW_MIN_ZOOM);

     // Orbital camera variables
     const [cameraMode, setCameraMode] = useState<string>(CAMERA_MODES.FOLLOW);
     const orbitalCameraRef = useRef({
         distance: CAMERA_CONFIG.ORBITAL_DISTANCE_MIN,
         height: CAMERA_CONFIG.ORBITAL_HEIGHT_OFFSET,
         angle: 0,
         elevation: CAMERA_CONFIG.ORBITAL_ELEVATION_DEFAULT,
         autoRotate: false,
         autoRotateSpeed: CAMERA_CONFIG.ORBITAL_AUTO_ROTATE_SPEED,
         lastUpdateTime: Date.now(),
         playerFacingRotation: 0
     });
     ...
     const calculateClientMovement = useCallback((currentPos: THREE.Vector3, currentRot: THREE.Euler, inputState: InputState, delta: number): THREE.Vector3 => {
         ...
         let worldMoveVector = new THREE.Vector3();
         const speed = inputState.sprint ? PLAYER_CONFIG.SPEED * PLAYER_CONFIG.SPRINT_MULTIPLIER : PLAYER_CONFIG.SPEED;
         let rotationYaw = 0;
     ...
     useEffect(() => {
         ...
         loader.load(
             ...
             (fbx) => {
                 ...
                 if (group.current) {
                     ...
                     try {
                         ...
                         console.error(`Error during fbx.traverse for light removal:`, traverseError);
                     }
                 }
             ...
             undefined,
             (error: any) => {
                 console.error(`Error loading model ${mainModelPath}:`, error);
             }
         );
     ...
     useEffect(() => {
         ...
         const handleMouseWheel = (e: WheelEvent) => {
             ...
             if (cameraMode === CAMERA_MODES.FOLLOW) {
                 const zoomChange = Math.sign(e.deltaY) * CAMERA_CONFIG.FOLLOW_ZOOM_SPEED;
                 targetZoom.current = Math.max(CAMERA_CONFIG.FOLLOW_MIN_ZOOM, Math.min(CAMERA_CONFIG.FOLLOW_MAX_ZOOM, zoomLevel.current + zoomChange));
             } else if (cameraMode === CAMERA_MODES.ORBITAL) {
                 const orbital = orbitalCameraRef.current;
                 const zoomChange = Math.sign(e.deltaY) * CAMERA_CONFIG.ORBITAL_ZOOM_SPEED;
                 orbital.distance = Math.max(CAMERA_CONFIG.ORBITAL_DISTANCE_MIN, Math.min(CAMERA_CONFIG.ORBITAL_DISTANCE_MAX, orbital.distance + zoomChange));
             }
         };
     ...
     const toggleCameraMode = useCallback(() => {
         ...
         if (newMode === CAMERA_MODES.ORBITAL) {
             orbitalCameraRef.current.playerFacingRotation = localRotationRef.current.y;
             orbitalCameraRef.current.angle = localRotationRef.current.y;
             orbitalCameraRef.current.elevation = CAMERA_CONFIG.ORBITAL_ELEVATION_DEFAULT;
         }
     }, [cameraMode]);
     ...
     // --- useFrame: Movement, Reconciliation, Camera Update, Animation ---
     useFrame((state, delta) => {
         const dt = Math.min(delta, 1 / 30);
         ...
         if (group.current && modelLoaded) {
             if (isLocalPlayer && currentInput) {
                 ...
                 const serverPosition = new THREE.Vector3(dataRef.current.position.x, dataRef.current.position.y, dataRef.current.position.z);
                 const unflippedServerPosition = serverPosition.clone();
                 unflippedServerPosition.x *= -1;
                 unflippedServerPosition.z *= -1;

                 const positionError = localPositionRef.current.distanceTo(unflippedServerPosition);
                 if (positionError > PREDICTION_CONFIG.POSITION_RECONCILE_THRESHOLD && cameraMode !== CAMERA_MODES.ORBITAL) {
                     localPositionRef.current.lerp(serverPosition, PREDICTION_CONFIG.RECONCILE_LERP_FACTOR);
                 }
                 ...
                 if (rotationError > PREDICTION_CONFIG.ROTATION_RECONCILE_THRESHOLD) {
                     currentQuat.slerp(reconcileTargetQuat, PREDICTION_CONFIG.RECONCILE_LERP_FACTOR);
                     localRotationRef.current.setFromQuaternion(currentQuat, 'YXZ');
                 }
                 ...
                 if (isDebugArrowVisible && scene) {
                     ...
                     if (debugArrowRef.current) {
                         ...
                         debugArrowRef.current = new THREE.ArrowHelper(
                             forwardDirection,
                             playerWorldPos.clone().add(new THREE.Vector3(0, 0.5, 0)),
                             arrowLength,
                             arrowColor
                         );
                         debugArrowRef.current.userData.isDebugArrow = true;
                         scene.add(debugArrowRef.current);
                     }
                 } else {
                     if (debugArrowRef.current && debugArrowRef.current.parent) {
         ...
         if (isLocalPlayer && group.current) {
             ...
             if (cameraMode === CAMERA_MODES.FOLLOW) {
                 const targetPosition = new THREE.Vector3(
                     ...
                     playerPosition.y + CAMERA_CONFIG.FOLLOW_CAMERA_HEIGHT,
                     playerPosition.z - Math.cos(playerRotationY) * zoomLevel.current
                 );

                 camera.position.lerp(targetPosition, Math.min(1, dt * CAMERA_CONFIG.FOLLOW_DAMPING));
                 const lookTarget = playerPosition.clone().add(new THREE.Vector3(0, CAMERA_CONFIG.FOLLOW_LOOK_HEIGHT, 0));
                 camera.lookAt(lookTarget);
             } else if (cameraMode === CAMERA_MODES.ORBITAL) {
                 const orbital = orbitalCameraRef.current;
                 const horizontalDistance = orbital.distance * Math.cos(orbital.elevation);
                 ...
                 camera.position.lerp(targetPosition, Math.min(1, dt * CAMERA_CONFIG.ORBITAL_DAMPING));
                 const lookTarget = playerPosition.clone().add(new THREE.Vector3(0, 1.5, 0));
                 camera.lookAt(lookTarget);
             }
         }
         ...
         // Server Animation Sync
         if (mixer && playerData.currentAnimation && playerData.currentAnimation !== currentAnimation && animations[playerData.currentAnimation]) {
             try {
                 playAnimation(playerData.currentAnimation);
             } catch (error) {
                 console.error(`Error playing animation ${playerData.currentAnimation}:`, error);
                 if (animations['idle'] && currentAnimation !== 'idle') {
                     playAnimation('idle');
                 }
             }
